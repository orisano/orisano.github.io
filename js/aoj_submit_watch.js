$(function() {
  var aojLib, appendSubmitLog, cache, getIntersection, submitLogTemplate;
  aojLib = new AOJLib();
  ...
  $.getJSON("users.json").done(function(json) {
    var hash_users, users;
    hash_users = location.hash.substr(1).split(",");
    users = hash_users[0] === "" ? json["users"] : getIntersection(hash_users, json["users"]);
  ...
  if (cache) {
    return appendSubmitLog(cache, 0, 30);
  }
});
